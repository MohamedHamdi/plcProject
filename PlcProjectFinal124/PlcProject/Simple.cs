...
namespace com.calitha.goldparser
{
    ...
    class Block
    {
        public int blockID, stratLine, endLine;
        public string name;
    }
    ...
    class itemDgv
    {
      ...
      public string lexeme { get; set; }
        ...
        public itemDgv(string t , string l)
       {
           token = t;
           lexeme = l;
       }
    ...
    public class MyParser 
    {
        ...
        public MyParser(string filename,ListBox l,ListBox l1 , ListBox l2,Label lbb,DataGridView d)
        {
        ...
        int blockID = 0;
        List<itemDgv> symbolTable = new List<itemDgv>();
        ...
        private void TokenReadEvent(LALRParser parser, TokenReadEventArgs args)
        {
            try
            {
                ...
                string lexeme = args.Token.ToString();

                dgv.ColumnCount = 2;

                
                itemDgv n = new itemDgv(args.Token.Symbol.Name,lexeme);

                if (symbolTable.Contains(n)==false)
                {
                    symbolTable.Add(n);
                ...
                int line = args.Token.Location.LineNr;
        ...
        void AddVariable(string n, string t, object v)
        {
            Variable va = new Variable();
            va.name = n;
            va.type = t;
            va.value = v;
            va.blockID = blocks[blocks.Count - 1].blockID;
            vars.Add(va);
        }

        bool blockNameExist(string x)
        {
        ...
        bool CheckIfDefined(string n,string type,int bId)
        {
            foreach (Variable item in vars)
            {
                if (n == item.name && type == item.type && bId == item.blockID )
                {
                    return true;
                }
            }
        ...
        private void ReduceEvent(LALRParser parser, ReduceEventArgs args)
        {
            ...
            {
                args.Token.UserObject = CreateObject(args.Token);
                string varName, varType, varValue,varName1, varType1, varValue1;
                

                switch (args.Rule.Lhs.Name)
                {
                    case "identification":



                        if (!args.Token.ToString().Contains("^^") && !args.Rule.ToString().Contains("<arthimetic>"))
                            {
                                varName = args.Token.Tokens[1].ToString();
                                varType = args.Token.Tokens[0].ToString();
                                varValue = args.Token.Tokens[3].ToString();
                                varValue1 = "";
                                varType1 = "@";
                            }
                        else if (args.Token.ToString().Contains("^^") && !args.Rule.ToString().Contains("<arthimetic>"))
                            {
                                varType = args.Token.Tokens[0].ToString();
                                varType1 = args.Token.Tokens[1].ToString();
                                varValue = args.Token.Tokens[5].ToString();
                                varValue1 = args.Token.Tokens[6].ToString();
                                varName = args.Token.Tokens[2].ToString();
                                varName1 = args.Token.Tokens[2].ToString();
                        ...
                        else
                        {
                            ...
                            foreach (var item in varValue)
                            {
                                for (int i = 'a'; i < 'z'; i++)
                                {
                                    if (item == (char)i) countt++;

                                }
                            }

                            if (countt == 0) varValue = "int";
                            else varValue = "string";


                            if (args.Token.ToString().Contains("^^"))
                            {
                                foreach (var item in varValue1)
                                {
                                    for (int i = 'a'; i < 'z'; i++)
                                    {
                                        if (item == (char)i) countt1++;

                                    }
                                }
                                if (countt1 == 0) varValue1 = "int";
                                else varValue1 = "string";
                            }

                            if ((CheckIfDefined(varName, varType, blockID - 1)))
                            {
                                something.Items.Add("variable is defined before");

                            }

                            else
                            {
                                if (varTypeTrue(varType, varValue,varType1,varValue1))
                                {
                                    AddVariable(varName, varType, varValue);
                                }
                                else
                                {
                                    something.Items.Add("variable Type is Not true");
                                }
                    ...
                    case "block": 
                        {
                            if (blockNames.Count == 0)
                                ...
                                blockNames.Add(args.Token.Tokens[0].ToString());
                              
                            
                            else
                            {
                                if (!blockNameExist(args.Token.Tokens[0].ToString()))  blockNames.Add(args.Token.Tokens[0].ToString());  
                                else tevlist.Items.Add("There is More than one block with the same Name ");
                            }
        ...
        bool varTypeTrue(string n,string t,string n1="@",string t1="")
        {
           
            if (n.Remove(0,1) == t && n1.Remove(0,1) == t1  ) return true;
            else return false;
        }
        ...
        {
            string message = "Unexpected Token : ' " + args.UnexpectedToken.Text + " '"+" In Line "+args.UnexpectedToken.Location.LineNr;
            pevlist.Items.Add(message);
            string message1 = "Expected Token : ' " + args.ExpectedTokens.ToString() + " '" ;
            something.Items.Add(message1);
